...
class Server:
    def __init__(self, parent = None):
        ...
        self.saveData = self.parent.saveData
        self.userInfo = self.parent.saveData["user"]

        #User Data
        self.userName = self.userInfo["name"]
        self.userId = self.userInfo["userId"]
        self.icon = self.userInfo["avt"]
        self.devID = self.userInfo["devId"]
        ...
        try:
            while True:
                try:
                    ...
                    Thread(target=self.verifyConnection,  args=(conn, addr), daemon=True).start()
                ...
                if not self.mainConn:
                    ...
                    if "--handShake--" in data:
                        ...
                        if data[1] not in self.blockUserList:                            
                            if self.parent.askIsAllowedToConnect(data):

                                if self.saveData["servSetting"]["lcenc"]:
                                    self.isEncryption = True

                                    key = self.parent.loginwindow.getKeysUsingDevID(data[2])
                                    if not key:
                                        print("Please Verify your id....")
                                    self.encryptKey = rsa.PublicKey._load_pkcs1_pem(key)

                                    
                                self.send(f"ok--{data[1]}---{self.isEncryption}")
                                if self.isEncryption:
                                    if self.checkEncryption():
                                        print("ooooooooooooooo==> Newtwork is encrypted.......")
    ...
    def checkEncryption(self):
        ...
        msg = self.mainConn.recv(1024)
        print(f"msg===> {msg}")

        # try:
        orginal_msg = rsa.decrypt(msg, self.decryptKey).decode()
        print(f"orginal_msg===> {orginal_msg}")
        if orginal_msg.split("---")[1] == "AllowEncryptionServerSide":
            msg = f"{self.parent.loginwindow.genSessionId(30)}---AllowEncryptionClientSide---{self.parent.loginwindow.genSessionId(20)}"
            encMsg = rsa.encrypt(msg.encode(), self.encryptKey)
            print(f"send encrypted message....")
            self.mainConn.sendall(encMsg)
            self.mainConn.settimeout(1.0)
            msg = self.mainConn.recv(1024)
            print(f"msg==> {msg}")
            if msg == "ok....":
                return True
        else:
            print(f"orginal_msg.split(---)[1]==> {orginal_msg.split('---')[1]}")
            return False
        # except:
        #     return False
        
    def sendFile(self, path):
        self.stillSending = True
        ...
        self.progObj = self.parent.sendFilesObj[path]
        st = f"(0 KB of {data_size_cal(self.size)} , 0.0 KB/s)"
        t = "Initializing"
        # from sendwin import createProgressBar
        # self.progObj = createProgressBar()
        self.progObj.update(timeSt=t, speed=st)
    ...
    def recv(self, bufSize=1024):
        if "~" in self.prevData:
            ...
            while True:
                try:
                    print(f"self.mainConn==> {self.mainConn}")
    ...
    def ActOnData(self, data): # Action Ferformed on the data
        ...
        if "--sendNewFile" in data:
            log("SendFileRequest")
            if self.parent.sendFilesList != []:
                path = self.parent.sendFilesList.pop()
                self.sendFile(path)
            else:
                self.stillSending = False
                self.parent.sendFileBut.config(command=self.parent.sendFilesCmd, fg="cyan2", text="Send Files")
    ...
    def doneOneFile(self, fileIdd):
        if self.processed1 and self.processed2:
            ...
            speed = f"({data_size_cal(self.sizeOfCurrentFile)} of {data_size_cal(self.sizeOfCurrentFile)} , {data_size_cal(sp)}/Sec)"

            self.progObj.update(value=self.sizeOfCurrentFile, timeSt=timeSt, timeStamp=t, speed=speed)
            self.parent.speedLab.config(text=f"Speed : {data_size_cal(sp)}/Sec")
    ...
    def speed(self, fun=None):
        if not self.stoptimer:
            ...
            ts = sz-pr # size data send in one second
